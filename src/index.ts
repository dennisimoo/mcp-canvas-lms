...
import { CanvasHttpServer } from "./http-server.js";
...
function parseArgs() {
  ...
  const parsed: { mode: 'stdio' | 'http'; port: number } = {
    mode: 'stdio',
    port: 8000
  };
  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i] === '--mode' && args[i + 1]) {
      ...
      if (isNaN(parsed.port) || parsed.port < 1 || parsed.port > 65535) {
        console.error(`Invalid port: ${args[i + 1]}. Must be a number between 1-65535`);
        process.exit(1);
      }
      i++;
...
Options:
  ...
  --port <port>    HTTP server port (default: 8000, only used in http mode)
  --help, -h       Show this help message

Examples:
  node build/index.js                          # Run in stdio mode (for AI tools)
  node build/index.js --mode http              # Run HTTP server on port 8000
  node build/index.js --mode http --port 3000  # Run HTTP server on port 3000

Environment Variables:
  CANVAS_API_TOKEN     Your Canvas API token (required)
  CANVAS_DOMAIN        Your Canvas domain (required)
  CANVAS_MAX_RETRIES   Max retry attempts (default: 3)
...
async function main() {
  ...
  try {
    if (cliArgs.mode === 'http') {
      ...
      const httpServer = new CanvasHttpServer({
        port: cliArgs.port,
        canvasClient,
        version: config.version
      });
      ...
      // stdio mode - MCP server for AI tools
